...
## 这是什么
 一个通过 **BLE 心率广播**（标准心率服务 UUID `0x180D`）实时读取手环/手表/心率带心率的双平台应用。
 设备端开启心率广播后，本应用扫描连接并显示实时 BPM，支持悬浮窗、状态栏常驻、
 历史图表、OBS 浏览器源叠加，以及 HTTP / WebSocket / Webhook 数据输出。

### 兼容设备
 任何支持标准 BLE 心率服务（0x180D）的设备，包括但不限于：
 - 小米手环 / 小米手表（开启心率广播）
 - 华为手环 / 华为手表（设置 → 心率广播）
 - 荣耀手环 / 荣耀手表
 - Amazfit / Zepp OS 手表（开启心率推送）
 - 佳明手表（开启 Broadcast Heart Rate）
 - Apple Watch（需配合第三方广播 App）
 - Polar / Wahoo / Coospo 等标准 BLE 心率带
...
- **OBS 浏览器源心率显示**：内置 HTML 页面，OBS 添加浏览器源填入服务器地址即可实时显示心率，支持自定义颜色、大小、字体等样式参数
- **悬浮窗显示心率**：在手机屏幕上显示心率，不受到应用内容的影响
- **状态栏常驻心率**：状态栏显示实时心率
...
2. 等待 **Gradle** 自动同步依赖
3. **构建并运行**：使用真机或模拟器（API ≥ 24）连接，点击工具栏中的 ▶ 运行按钮
...
[MIT](./LICENSE) © 2026 XiaochangXu & TheSixPasserby
