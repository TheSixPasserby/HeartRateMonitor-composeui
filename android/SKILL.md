...
description: 项目架构契约、编码规范、验证基线与踩坑指南
---
...
- 2026-09-04 心率记录改为三档模式（`SettingsKeys.RECORDING_MODE`：`RecordingMode.OFF/AUTO/MANUAL`，默认 MANUAL；旧 `HISTORY_RECORDING_ENABLED` 布尔键仅作迁移读取，业务禁直接用）：连接成功仅 AUTO 自动建会话；MANUAL 由首页 `RecordControl` + 控制面 `startRecording()/stopRecording()` 驱动；`record()` 懒创建仅限 AUTO；`recordingStartTime` 暴露于 HeartRateRepository（断开/蓝牙关/服务重建清零）；手动停止走 `endSessionDiscardIfEmpty`（空会话不落历史）；功能设置页为三档选择器（OFF 直写、MANUAL/AUTO 经性能警告确认）；涉及测试已同步换键。**本地无构建环境未执行验证，变更面大，必须真机回归后再发版**；
...
- 通知文案、统计卡 BPM、时间（`14:30`→`१४:३०`）、图表刻度（`80`→`८०`）、
  小数（`12.5`→`१२.५`，阿拉伯语下小数点变 `٫`）全部显示本地数字

### 根因

Android 的 locale 敏感格式化在这些语言下使用本地数字系统（天城文 ०-९ 等）渲染数字，
共四个层面（2026-08 三轮审查才找全，务必逐一对照）：

1. **资源层**：`getString(R.string.xxx, intValue)` + `%N$d` 占位符
   （`String.format` 语义，按当前 Locale 格式化整数）
2. **Kotlin 层**：未传 Locale 的 `String.format("%.1f", v)`、`"%.1f".format(v)`、`%02d`
3. **日期层**：`SimpleDateFormat(pattern, Locale.getDefault())`——pattern 中的数字部分
   同样走本地数字系统
4. **三方库层**：Vico 图表轴未显式传 `valueFormatter` 时，默认
   `DecimalValueFormatter` 内部走默认 locale 渲染刻度

### 正确做法（本项目强制，2026-08 已全量整改 159 个文件）

1. **字符串资源**：数字占位符一律 `%N$s`（禁止 `%N$d`/`%N$f`），
   代码侧传 `int.toString()`——Kotlin `Int.toString()` 恒输出 ASCII 十进制，不受 Locale 影响
2. **代码内直接格式化**：`String.format(Locale.US, "%.1f", v)`，禁止省略 Locale 参数
3. **时间格式化**：`SimpleDateFormat(pattern, Locale.US)`（纯数字 pattern 在任何 locale
   下格式相同，唯一差异就是数字系统，改后仅数字恒 ASCII）
4. **图表轴标签**：必须显式传 `CartesianValueFormatter { _, value, _ -> value.toInt().toString() }`
   （参考 `RealtimeChart.kt` 与 `ChartComponents.kt` 既有写法）
5. **对外协议数据**（webhook JSON、上报载荷）：`String.format(Locale.US, ...)`
   （`WebhookRepository` 既有做法，2026-08 之前即正确）
...
- 修改翻译文件占位符后 → 校验各语言占位符编号/数量与默认 `values` 一致
  （不一致会在对应语言下运行时抛 `MissingFormatArgumentException`）
