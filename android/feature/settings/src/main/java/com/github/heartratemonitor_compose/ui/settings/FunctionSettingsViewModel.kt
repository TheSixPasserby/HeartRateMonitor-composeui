...
class FunctionSettingsViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            settings.settings.collect { s ->
                setState {
                    ...
                    navAnimationDisabled = s.navAnimationDisabled
                    )
                }
            }
        }
    ...
    override suspend fun handleIntent(intent: FunctionSettingsIntent) {
        when (intent) {
            ...
            is FunctionSettingsIntent.SetNavAnimationDisabled ->
                settings.set(SettingsKeys.NAV_ANIMATION_DISABLED, intent.enabled)
        }
    }
}
...
sealed interface FunctionSettingsIntent {
    ...
    data class SetNavAnimationDisabled(val enabled: Boolean) : FunctionSettingsIntent
}
...
internal fun initialFunctionSettingsUiState(settings: SettingsRepository): FunctionSettingsUiState = FunctionSettingsUiState(
    ...
    navAnimationDisabled = settings.get(SettingsKeys.NAV_ANIMATION_DISABLED)
)
